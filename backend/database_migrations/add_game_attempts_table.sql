-- Migration: Add game_attempts table for storing user game attempt history
-- This table stores individual attempts made by users on games/stories with scores and timestamps
...
CREATE INDEX IF NOT EXISTS idx_game_attempts_story_id ON game_attempts(story_id);
CREATE INDEX IF NOT EXISTS idx_game_attempts_start_date ON game_attempts(start_attempt_date DESC);
CREATE INDEX IF NOT EXISTS idx_game_attempts_end_date ON game_attempts(end_attempt_date DESC);
CREATE INDEX IF NOT EXISTS idx_game_attempts_user_story ON game_attempts(user_id, story_id);
CREATE INDEX IF NOT EXISTS idx_game_attempts_user_end_date ON game_attempts(user_id, end_attempt_date DESC);
